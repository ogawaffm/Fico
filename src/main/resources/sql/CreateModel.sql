...
CREATE VIEW FILE_DUPLICATE AS (
    SELECT FILE.*, DUP.DUP_COUNT, DUP.DUP_SIZE, DUP.DUP_SAVABLE,
           CASE WHEN IS_DIR = TRUE THEN
               'RMDIR "' || PATH || '\' || NAME || '" /S /Q'
           ELSE
               'DEL "' || PATH || '\' || NAME || '"'
           END DUP_DROP
    ...
    JOIN (SELECT CHECKSUM, COUNT(CHECKSUM) DUP_COUNT, SUM(SIZE) DUP_SIZE,
          ...
          FROM FILE
          WHERE CHECKSUM IS NOT NULL AND CHECKSUM <> X'00'
          GROUP BY CHECKSUM
          HAVING COUNT(CHECKSUM) > 1
    ) AS DUP
    ON FILE.CHECKSUM = DUP.CHECKSUM
    ORDER BY DUP.DUP_SAVABLE DESC, FILE.CHECKSUM, FILE.NAME
)